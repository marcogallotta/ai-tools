...
def release_fixture():
    ...
    return ResolvedRelease(
        version="fixture-v1", commit="fixture-commit", root=FIXTURE_DIR,
        protocols={
...
def task(notes, section="research"):
    ...
    return {
        "gid": "task", "name": "Task", "notes": notes,
...
def test_initial_prepare_routes_to_opposite_family_and_moves(tmp_path):
    ...
    result = app.execute("prepare", agent="claude", submission_id=sid, file_path=write_candidate(tmp_path, COMPLETE_NOTE))
...
def test_prepare_reports_multiple_validation_failures_and_stays_drafting(tmp_path):
    ...
    sid = start(app, "initial")
    result = app.execute("prepare", agent="claude", submission_id=sid, file_path=write_candidate(tmp_path, "bad"))
...
def test_small_change_preserves_verification_and_exemptions(tmp_path):
    ...
    result = app.execute("prepare", agent="claude", submission_id=sid, file_path=write_candidate(tmp_path, changed))
    assert result["code"] == "VALIDATION_FAILED"
...
def test_large_exemption_change_requires_revision(tmp_path):
    ...
    failed = app.execute("prepare", agent="claude", submission_id=sid, file_path=path)
    assert any(e["rule"] == "exemption_revision_required" for e in failed["errors"])
    passed = app.execute("prepare", agent="claude", submission_id=sid, file_path=path, exemption_revision="Marco, 2026-07-21: removed exemption")
...
def test_research_handoff_retry_skips_validation_and_only_moves(tmp_path):
    ...
    first = app.execute("prepare", agent="claude", submission_id=sid, file_path=write_candidate(tmp_path, COMPLETE_NOTE))
