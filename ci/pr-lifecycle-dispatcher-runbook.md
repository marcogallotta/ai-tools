...
A draft PR that explicitly carries `IMPLEMENTATION EVIDENCE PENDING: <evidence>` is `IMPLEMENTATION CONTINUATION REQUIRED`, not Review/Integration/local certification. The dispatcher reuses the existing Implementation/fix consumer. If no current exact-head `phase=implementation`/`fix` advisory lease is active, it first writes a durable `dish-implementation-continuation:v1` handoff on the same PR, then claims `phase=implementation` and dispatches the exact repository + owning Asana task + authorized branch + base ref/SHA + PR/head assignment plus named evidence. The PR body must durably declare one owning task and `Exact source/base SHA: <sha>`; missing or conflicting assignment identity fails closed. The handoff follows `ci/implementation-handoff.md`. It is dispatch authority for that exact lineage, **not** an implicit takeover of an active local `tools/agent-worktree` claim: the consumer must reconcile the local claim and either resume/handoff to its owner, or perform an explicitly authorized stale-owner transfer using the exact current-claim compare-and-set. The only human-facing message for this state is `PR #N still needs Implementation to finish <evidence>.`
...
or `--implementation-fixer`. The command receives `dish-pr-fix-dispatch-v1` JSON on standard input containing the exact repository, owning Asana task GID, authorized branch, base ref + durable authoring base SHA, PR URL/number, expected/blocked head SHA, the current lifecycle snapshot, and either the authoritative formal BLOCK review or the structured PR-owned CI diagnosis. The PR body must durably declare one owning task and `Exact source/base SHA: <sha>`; missing or conflicting assignment identity fails before a fix lease is posted. The consumer must follow `dish/docs/agents/implementation.md` and `ci/implementation-handoff.md`, reconcile the exact `tools/agent-worktree` assignment/claim before touching local state, update only the existing authorized PR branch, and re-read GitHub before semantic work. Matching Asana task identity on another branch/PR is not authority. A CI-driven semantic fix changes head identity and therefore requires substantive Review on the new head.

Before launching the consumer, the dispatcher validates the complete assignment tuple and then writes an exact-head `phase=fix` advisory lease. A fresh `phase=fix` or `phase=implementation` lease on the current blocked head prevents duplicate dispatcher launches, but does not transfer or override a local agent-worktree claim. A head move immediately invalidates the old review and lease; the dispatcher never launches a fix consumer for a BLOCK that is no longer on the current head. If the configured command fails synchronously, the dispatcher releases its lease so recovery is not deadlocked.
...
Before notifying Marco about either local action, the dispatcher first writes the complete exact-head handoff to the PR with a `dish-local-handoff:v1` marker. For `kind=implementation`, that durable handoff includes the exact repository + owning task + authorized branch + base ref/SHA + PR/head assignment and points to `ci/implementation-handoff.md`; the local worker must reconcile the exact agent-worktree claim before touching prepared state. If the local implementation action changes the source head, the prior Review and completion marker are stale and the new head returns to Review/recheck under the normal rules.
