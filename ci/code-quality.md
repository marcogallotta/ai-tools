...
`implementation-finalize`, `scripts/pr_gate.py review-ready`, and lifecycle Review discovery consume the same exact-head author-result predicate. Enablement is monotonic across the authoring boundary: enabled on either the comparison base or candidate head means enabled. A direct ready transition cannot bypass a missing, stale, duplicate, mismatched, or unacceptable result. When both policies are disabled, the predicate is deterministically nonblocking.
...
The recovery machinery lands while this value remains `false`. Activation is a separate ordinary reviewed PR. Before activation may merge, one fixed draft activation head must prove both fresh ChatGPT and fresh local author routes: omission refusal, direct-ready refusal, offline acquisition and analyzers, successor-head invalidation, rerun acceptance, and trusted attestation. Failure leaves main disabled.
