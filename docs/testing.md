...
The shared PGlite fixture does not trust a raw TCP accept as readiness because that probe can
temporarily consume the socket shim's connection slot. It waits for the launcher's database-ready
server signal, then proves two independent PostgreSQL protocol connections. The generated socket
server allows sixteen connections because its one-connection default can reject a rapid reconnect
while prior-client cleanup is still queued. Recognized failures before the fixture yields may be
retried with a fresh directory and port.
Once a test starts, its exceptions are never intercepted or rerun.
