...
test("admin contract preserves a durable Human Review question beyond 4096 characters", () => {
  const value = payload();
  const question = `Choose between the documented operator alternatives after reviewing this full context: ${"material evidence and consequence; ".repeat(160)}`;
  assert.ok(question.length > 4096);
