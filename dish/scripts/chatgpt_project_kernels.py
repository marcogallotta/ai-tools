...
def _render_context_dependencies(s,role):
 ...
 if preload:
  ...
  primary='role-index standing contracts' if preload['role_index_contracts'] else 'current role contract'
  lines.append(f'Startup/re-ground context: {primary} + {extra}. Read-only; grants no role/mutation/Review/Integration/merge/production authority.')
...
REQUIRED_EVAL_IDS|={'action-ready-stops-chatgpt-archaeology','material-unknown-still-allows-bounded-targeted-investigation','chatgpt-manual-never-claims-hard-investigation-closure'}
