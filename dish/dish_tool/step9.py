...
# Compatibility wrappers keep the long-standing step9 import surface while the
# read-only authority now lives in a neutral component.
from .submission_authority import (
    approved_signoff as _authority_approved_signoff,
...
def _approved_signoff(conn: sqlite3.Connection, operation_id: str):
    return _authority_approved_signoff(conn, operation_id)


def _signed_identity(conn: sqlite3.Connection, operation_id: str) -> str:
    return str(_authority_approved_signoff(conn, operation_id)["signed_identity"])
