...
`manifest.max_project_settings_chars` is a repository compatibility budget for the **exact installable Project-settings payload**, not a published OpenAI Projects limit. Its current value is **8,000 characters**, inherited from the repository's existing conservative operating assumption. The canonical composer validates the actual production or TEST payload and reports base-kernel, TEST-metadata, total, remaining, and excess character counts. Kernel length alone is diagnostic and can never make an oversized composed payload green.

Generation checks deterministic production and TEST payload fixtures for every persistent role against the current compatibility budget. Arbitrary TEST identity strings are checked at their actual size; they are never truncated or promised unlimited headroom. Changing the numeric compatibility budget away from 8,000 requires explicit evidence from either a bounded empirical Project save/load/readback or an official Project-limit publication. Live Project/browser proof is therefore not a standing gate for ordinary changes that remain under the existing budget.
...
python3 dish/scripts/chatgpt_project_kernels.py refresh --base-manifest <current-main-manifest> --base-source <current-main-source> --source dish/docs/chatgpt-projects/source.json
...
`refresh` is the normal one-command Project update: it reconciles the canonical manifest/history, renders all tracked kernels, runs `check`, and only then reports the paste-ready files. The lower-level `reconcile`, `render`, and `check` commands remain available for diagnosis and concurrent-lineage handling. `check` validates source/rendered-version binding, current role topology, generated files, the exact composed Project-settings compatibility budget (including supported production and TEST payload fixtures), semantic-history configuration, required-version reachability, current-edge classifications, proof/correction/retirement metadata, and the complete approved eval contract set. It does **not** report behavioral adherence. `prepare-eval` emits oracle-free cases containing the exact current Project kernel and prompt. `eval` judges structured results from one newly created ChatGPT Project chat per case, either from a recorded result bundle or from an operator-supplied runner command invoked separately for every case.
...
The repository keeps the complete approved matrix — currently 113 scenarios / 217 role-expanded cases — as deterministic harness coverage. `prepare-eval` emits the full matrix, and action-bearing cases keep their machine-verifiable observation requirements.
