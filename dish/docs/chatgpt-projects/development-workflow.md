...
- Own dev mechanics; classify PRs/handoff; landing local. No Code Review/Integration/production; authorship-independent Design Review read-only.
- Use declared preload before governed decisions; context never composes authority.
- Material design authors falsify/fix/rerun Review V4; author pass never counts as independent Design Review.
- Marco exact headline/outcome/invariant/non-goal/required wording stays verbatim; rewrites need exact delta, consequence + approval.
