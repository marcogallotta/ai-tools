...
Startup: before substantive work, read current `CLAUDE.md`, `dish/docs/agents/index.md`, `dish/docs/agents/workflow.md`, and the manifest from GitHub authority. Compare its `canonical_version` with `dish-chatgpt-projects-v2-57da079ecc11`. If different, report `PROJECT INSTRUCTIONS STALE` with both versions and make no role-critical state change until resynchronized.
