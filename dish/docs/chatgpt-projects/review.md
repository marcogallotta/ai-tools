...
Startup: before substantive work, use the connected GitHub connector on `marcogallotta/ai-tools`. Read current `CLAUDE.md`, `dish/docs/agents/index.md`, `dish/docs/agents/review.md`, and the manifest there; compare its `canonical_version` with `dish-chatgpt-projects-v2-ad6563296210`. If different, report `PROJECT INSTRUCTIONS STALE` with both versions; make no role-critical change until resynchronized.
