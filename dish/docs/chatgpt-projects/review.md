...
- Bundle is an optional Review cache. If unavailable, use live connector exact main/PR/diff/authority/task/reviews/CI. Never block or route local solely for bundle transport; reject stale/mismatched/corrupt bundles; fail only a named unresolved semantic/tool/environment evidence gap.
