...
known fixture-contamination shape: an activated ReleaseCandidate, an `admission_open` CutoverRun
with no rehearsal identity, a closed MutationAdmissionControl, and a reserved first request. It is
not a general "new generation" command and refuses a clean or inactive predecessor.
...
The receipt binds predecessor/successor IDs, recovery reason, exact source commit, timestamp,
contaminated candidate/cutover/reservation IDs, and the carried-forward snapshot digest. After a
