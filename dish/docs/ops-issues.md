...
| Legacy-writer inventory — enforcement (library logic) | 2026-08-05 (directly code-verified) | `validate_writer_fence_observation()` (`release_validation.py:310-382`) and `mark_fenced()`/`activate_authority()` (`cutover_control.py:429-513`); commits `1d4729b`, `b7d0d0f`. Exact set-equality against `required_writer_inventory`, fails closed on `None`/empty/blank/duplicate, correct `missing_writer_targets`/`extra_writer_targets` diagnostics, threaded through verify → mark-fenced → activate. CLI wiring gap remains — see Partial table. |
...
| --- | --- | --- | --- | --- | --- |
| Legacy-writer fence CLI wiring | Must-fix | Mixed | Easy | 2026-08-05 (directly code-verified) | `scripts/dish-pg-release` never passes `required_writer_inventory` to `writer-fence-verify`/`cutover-mark-fenced`/`cutover-activate` (grepped, no match) — every real invocation hits the `None` branch and fails closed unconditionally. Enforcement logic itself is correct (see Done table); this just isn't wired to gate on the real inventory (`dish-service-prod.service`, `dish`, `dish-admin`) yet. No production effect today since cutover authority isn't live. |
