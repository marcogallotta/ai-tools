...
| Shared legacy service/local process lock | 2026-08-05 (directly code-verified) | `ServiceProcessLock` guards the service; `DatabaseProcessLock` guards local `dish` and `dish-admin` access to the governed SQLite database (`dish_service/__main__.py`, `dish_service/cli.py`, `dish_service/admin_cli.py`). |
