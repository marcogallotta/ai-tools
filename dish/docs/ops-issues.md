...
| Migration-target helper current head | 2026-08-09 (directly code-verified) | Release/bootstrap/acceptance helpers derive or assert `0035_persistence_constraint_integrity` through `ALEMBIC_HEAD`/`DEFAULT_SCHEMA_HEAD`; the Stage 6 runbook names the same checked-in head. |
...
| Stage 6 runbook migration-head claim | 2026-08-09 (directly doc-verified) | `docs/database-backend-stage6-runbook.md` consistently names `0035_persistence_constraint_integrity` as the checked-in target; historical migration references remain explicit where needed. |
...
| Dark-launch rollout-order stall: one permanently-failed delivery blocks all later evidence | Later | Mixed | Medium | 2026-08-09 (implementation documented; end-to-end production verification pending) | Delivery claiming is strictly rollout-ordered (`claim_delivery` in `dish_pg/transition.py`): an envelope cannot be claimed while any earlier-rollout-sequence delivery for the same baseline is not yet `delivered`. A `failed` delivery is terminal and nothing currently converts it back to `delivered` except a full retry. Live consequence observed 2026-08-08: one `prepare` delivery failed with "no unique target operation binding," permanently blocking 15 newer captured commands from ever being evaluated. No operator tool exists to skip/void a permanently-stuck delivery and unblock what's behind it (`skip_delivery` in `dish_pg/transition.py` requires an active claim, so it can't be invoked on an already-terminal `failed` row). The import-history fix above (task #61) has landed and is host-verified. Task #62 ("decoupling shadow-worker identifier bindings from sibling deliveries," the direct fix for this stall's root cause) is implemented in two patches — an operation-family resolver (Patch A) and lease/verification_cycle/planning_challenge/abandonment resolvers (Patch B) — both committed and deployed to production (`dish-service-prod.service`, `dish-shadow-worker.service`) and passing governed test lanes (smoke, dark-launch/shadow governed lane, native PostgreSQL certification). Real production traffic exercising the new resolvers end-to-end has not yet been observed; this row stays open until that live-traffic verification completes. |
