...
| Agent `inspect` request identity and replay | 2026-08-05 (directly code-verified and tested) | `inspect` remains verification-semantic-compatible but is consequential because it writes `dish_inspect_facts`; Action/private metadata now requires request identity, exact replay returns the first stored result, changed payload reuse conflicts, and the focused regression proves no duplicate evidence. Planner-selected focused tests: 164 passed after one stale-doc assertion was corrected. |
| `apply-proposal` client request identity | 2026-08-05 (directly code-verified and tested) | Private and Action clients now share the canonical replay-safe command inventory, generate a request UUID when omitted, preserve an explicit UUID, and the CLI accepts `--request-id`. The Action/service regression proves approval stays separate, exact replay does not repeat the backend write or create an extra Verification cycle, and changed-payload reuse conflicts. Planner-selected focused tests: 138 passed. |
