...
State is reconstructed from the immutable generation plus the durable ordered event stream. Invalid transitions, duplicate events, identity mismatches, invalid approval provenance, and successor-lineage disagreements are contradictions; they do not rewrite or replace the design bytes. `SUPERSEDED` and `CANCELLED` are terminal.

A valid `MARCO_APPROVED` event carries `material_delta_set_sha256` plus a reference and digest for the durable human decision. The event actor/account field is attribution only and is never sufficient proof of Marco approval.

## Marco approval provenance

Consequential approval uses independent durable human-decision provenance rather than service actor metadata.

The event binds `human_decision_ref` plus `human_decision_sha256`. Reconstruction accepts the approval only when the caller also supplies an independently recovered `dish-human-decision-provenance:v1` record for that reference and all of these values agree exactly:

- decision kind is `MARCO_APPROVAL`;
- `decided_by` is Marco;
- exact Review V2 generation identity;
- exact `material_delta_set_sha256`;
- exact durable decision digest.

Missing provenance, actor-only attribution, an unrecovered decision reference, or any identity/digest/delta mismatch surfaces `invalid-marco-approval-provenance` and the approval event does not change state. This preserves the operator-provenance rule that an agent acting through Marco's account cannot manufacture human authorization.

Cumulative-drift baseline selection uses the same provenance-aware reconstruction, so an actor-only or agent-minted `MARCO_APPROVED` event cannot become the approved baseline.
...
A valid `SUPERSEDED` event must name `successor_generation_id`. Lineage validation reconciles that exact event reference with the actual immutable child record. If G5 records `SUPERSEDED -> G7` while the actual child is G6, the disagreement is a contradiction; event state cannot silently override immutable predecessor/successor lineage.
...
A replacement independent reviewer may issue `UPHOLDS`, `NARROWS`, `REFRAMES`, or `WITHDRAWS` against the exact inherited challenge. Any agent included in cumulative material authorship of that candidate is not independent and cannot clear that candidate. The challenger is also not the independent reviewer for its own challenge, even when the challenger did not materially edit the candidate.
