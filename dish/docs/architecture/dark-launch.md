...
- TEST fixture-contamination recovery may roll the authority generation only through the dedicated TEST-only rollover transaction, and only when operator-supplied exact contaminated Stage 6 identities also match the persisted fixture-specific incident provenance; lifecycle state shape alone is never sufficient. The accepted incident evidence includes the known lifecycle-corrupt variant where an initial `assembling` candidate coexists with the exact later fixture rows, but recovery never repairs or advances that predecessor candidate in place. Stable project/section/task identities and aliases stay in place, current generation-scoped task/registry authority is freshly materialized, the contaminated predecessor and all of its Stage 6/reservation/shadow evidence remain untouched, and the successor starts with no release/admission authority plus a fresh effects-disabled projection epoch and shadow baseline.
