...
- The clean-break decision is ratified: the frozen final Asana source boundary ends Asana involvement, and rollback burn disables external projection rather than entering a read-only-Asana phase. Bridge-only Asana correlation gaps therefore do not need exhaustive post-burn resolution; PostgreSQL's own command/replay/audit correctness remains the cutover bar.
...
`postgresql-cutover.md`, "Settled cutover decisions," is the sequencing authority for the ratified clean break. Historical projection/reconciliation evidence remains retained, but it is not post-burn runtime authority.
