...
Its trust boundary is sealed canonical Review V2 reconstruction: the helper consumes the existing generation, event, and human-decision-provenance records plus independently recovered snapshot, Review, and exact decision-payload bytes, and recomputes every binding against the full four-part generation identity. Authorized semantic classification remains an input, but caller-created wrapper schemas, loaders, evidence fields, and internal consistency are not proof. Earlier malformed provenance is repairable metadata only when a later exact valid record independently establishes the same approval.
