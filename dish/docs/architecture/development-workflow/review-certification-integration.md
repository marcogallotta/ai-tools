...
Scope containment separates finding disposition (`BLOCKER` / `FOLLOW-UP` / `MATERIAL UNKNOWN`) from remedy scope (`S0 LOCAL` / `S1 EXPANSION` / `S2 GOVERNING CHANGE`). The governing baseline is original Marco outcome/non-goals plus explicit amendments/supersessions and pre-existing binding repository invariants; candidate-authored mechanisms and reviewer remedies do not become governing authority merely by appearing in a frozen/accepted generation.

Only BLOCKER × S0 enters ordinary correction. S1 gets fresh necessity review before successor authoring, S2 requires Marco, and material unknown is researched while only the affected publication boundary stays blocked. Code Review judges the whole accumulated first/successor head (and related stacks implementing one slice), while atomic landing is justified only by a concrete partial state that violates a binding invariant or falsely reports success.
