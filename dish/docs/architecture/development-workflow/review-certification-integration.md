...
Its trust boundary is sealed canonical Review V2 reconstruction issued inside the concrete repository-owned Asana resolver. The caller supplies exact durable task/generation-story/independent-Review-story GIDs, never canonical records or evidence bytes. The resolver reads the complete task history, proves story membership, recovers matching generation/event/human-decision records and referenced snapshot/Review/exact-decision payloads, derives material authorship, and recomputes every binding against the full four-part generation identity. Authorized semantic classification remains an input, but caller-created canonical objects, payloads, source lists, wrapper schemas, loaders, evidence fields, and internal consistency are not proof. Earlier malformed provenance is repairable metadata only when a later exact valid record independently establishes the same approval.
