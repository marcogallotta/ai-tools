...
Semantic role and execution host are separate. ChatGPT uses connected GitHub/Asana capabilities and repository-owned remote publication paths. Its GitHub authority begins at the Connector selection route; actions returned from that route remain Connector operations even when their namespace or protocol description contains `Github` or MCP wording. Claude Code and Codex use a live checkout plus native Git and the repository-owned worktree lifecycle. Worker is an execution mode whose authority comes from the routed standing role, not from its name. Final V1-A Integration landing remains local-only.
