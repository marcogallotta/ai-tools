...
`audit` is a separate read-only confidence surface. It compares the configured Cooking-project population with durable Dish-known identities and classifies each Dish/task rather than treating every placement difference as corruption. It is intended for population confidence work, not for deciding a legal workflow transition. `inspect --verbose <dish>` is the bounded per-Dish diagnostic drill-down: normal inspect remains consequence-first, while verbose mode exposes durable operation/cycle/request/effect/lease/revocation/proposal/history evidence needed to explain the compact result.
