...
| private frontend product or delivery work | [`frontend.md`](frontend.md), [`frontend-imp.md`](frontend-imp.md), [`frontend-stage2-runtime-decisions.md`](frontend-stage2-runtime-decisions.md), [`frontend-test-deployment-readiness.md`](frontend-test-deployment-readiness.md), the current Gate A packet for authentication/runtime integration, [`frontend-db-migration-reconciliation.md`](frontend-db-migration-reconciliation.md), and the current Gate B source map for canonical board/detail integration |
...
command so Marco never has to infer an approval from raw shell arguments. Legacy `admin_command*`
and directive fields are compatibility views of
...
one authenticated mutation rejection to the exact candidate, target, manifest digest and a proof
