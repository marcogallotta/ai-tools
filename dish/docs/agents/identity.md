...
`hooks/agent-reground` never bootstraps a missing identity. A missing, stale, malformed, ambiguous, or conflicting identity/provenance record leaves post-compaction recovery unresolved, but does not globally block tool use. Operations that require this identity or launch provenance still fail closed at their own consequential mutation boundary. Launch provenance is evidence that this host instance is the named session; it still does not create role, task, branch, Review, or Integration authority.
