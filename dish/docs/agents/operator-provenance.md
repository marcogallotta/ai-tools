...
- Immediately before any governed write in any other Dish-prefixed Asana project, freshly read and apply `dish/docs/agents/asana-v2-project-mode.md`'s registry and `lifecycle_state` for that exact project GID. A `structural-repair-pending`/`migration-pending` row or an unregistered project gets zero governed V2 mutation.
