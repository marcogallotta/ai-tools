...
PROFILE: manual-multi-role-worker-r2
...
The long-horizon qualification is executable rather than text-presence testing. `qualify_worker_late_action` evaluates resume/adopt, semantic publication, draft→review-ready, final handoff, Code Review verdict, Design Review verdict, and override-sensitive actions from the accepted attempt plus existing persistent identity/fence/readback/independence/override facts. `fresh_packet_loaded` is deliberately non-authoritative; tests drive repeated role switches and every late action with that packet omitted, proving each action is either rejected or safe solely from those persistent/existing gates. This qualifier is evidence for the profile contract, not a new writer/admission control plane.
