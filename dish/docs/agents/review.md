...
The helper accepts authorized semantic classification, but human-required mechanical sufficiency can be issued only by its repository-owned Asana admission resolver. Callers provide only the exact task, generation-story, and independent-Review-story GIDs; the resolver reads the complete task story history itself, proves those stories and every referenced snapshot/decision are members of that task, recovers all matching generation/event/human-decision records, derives material authorship, recomputes payload digests, and preserves the complete four-part generation identity. The ordinary admission surface does not accept an evidence object for human-required classes. Caller-created canonical or even internally sealed objects, payload bytes, source lists, loaders, wrapper schemas, booleans, role claims, or hashes therefore cannot become admission evidence. Earlier malformed provenance remains historical evidence and can be repairable metadata only when a later exact valid record independently establishes the same approval.
