...
The helper accepts authorized semantic classification, but mechanical sufficiency comes only from sealed canonical Review V2 reconstruction over the existing generation, event, and human-decision-provenance records plus independently recovered durable payload bytes. It recomputes snapshot, Review, and exact decision-payload digests and preserves the complete four-part generation identity. Caller-created wrapper schemas, loaders, booleans, role claims, hashes, or internally consistent claim objects are not admission evidence. Earlier malformed provenance remains historical evidence and can be repairable metadata only when a later exact valid record independently establishes the same approval.
