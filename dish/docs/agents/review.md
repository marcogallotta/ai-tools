...
**Live pilot only** — for the exact task/Review lineage IDs recorded under LIVE PILOT on task `1218204649733269`; inert for every other lineage and before those IDs are recorded, where the paragraph above and the pre-trial rules elsewhere in this file govern unchanged: separate the **governing baseline** (original Marco outcome/non-goals, explicit amendments/supersessions, pre-existing binding invariants) from candidate-selected mechanism and reviewer remedy, which stay non-authoritative even inside a frozen/accepted generation. Independently classify finding disposition — `BLOCKER` (current V1 cannot satisfy a governing outcome/invariant), `FOLLOW-UP` (real but safe outside current V1), `MATERIAL UNKNOWN` (evidence could change finding/scope/accepted slice) — and remedy scope — `S0 LOCAL` (same accepted semantic surface, no new interface/authority/durable state/mechanism class/dependency/workstream), `S1 EXPANSION` (materially changes one of those but may be necessary for V1), `S2 GOVERNING CHANGE` (changes/exceeds Marco-approved outcome/non-goals or accepted risk/cost/authority boundary); severity and scope never conflate. Route `BLOCKER × S0` through the current correction path and fresh exact-candidate Review; `BLOCKER × S1` needs one fresh context-free necessity review proving the minimum cohesive V1 remedy before successor authoring, or split the remainder to follow-up (unresolved disagreement goes to Marco); any `S2` goes to Marco; `FOLLOW-UP` stays outside current V1 absent separate promotion; `MATERIAL UNKNOWN` blocks only the affected publication until classified/routed. Judge the entire accumulated first/successor candidate/stack, reassessing related S0 changes cumulatively rather than only the latest delta, and treat work as atomic only when a concrete partial state would violate a binding invariant or falsely report success — never file count, conceptual relatedness, or a preference for one final switch. Security, performance, reliability, or correctness may justify `S1` only when evidence proves a governing failure, smaller routes are insufficient, and the remedy is the minimum viable one.

Design Review may discover and BLOCK any material design defect, stating the violated outcome or constraint. That discovery does not grant authority to author the replacement design. A suggested remedy is advisory and non-authoritative; a material correction returns to Design as a revised exact candidate — or, on a pilot lineage, follows the routing above — and then receives fresh independent Design Review. The reviewer that blocked the prior candidate must not edit/fix the successor and then independently PASS it.
...
## Human Review timing and deferral

Human steering is consequence-specific. When research, design, or Review reaches a genuine Marco-owned choice that materially determines operator experience, architecture/control-plane shape, intent, authority/trust, irreversible compatibility, security/risk, product behavior, or major cost, conduct Human Review **while that choice is live and before further design/freeze work on it**. Routine, reversible, agent-resolvable design stays agent-owned. Human Review is the current concrete Marco-only decision affecting that semantic scope, never generic ceremony for difficult work.

The review must be substantive: independently state the real decision, minimum evidence, meaningful alternatives/tradeoff, recommendation when defensible, and material consequence. Merely repeating Marco's requested outcome and asking `approve/reject` fails this boundary. If Marco answers in the interaction, persist the exact answer immediately on the owning task. A later frozen candidate binds that durable decision and does not request ceremonial re-approval unless it introduces a material consequential delta outside the recorded scope; such a delta triggers a new immediate Human Review.

Only Marco's explicit deferral such as `not now` may park the exact unresolved decision in `Needs Human Review`. Persistence and presentation are one control flow: record the exact pending revision, project the lifecycle state, then present that same decision before ending the interaction. Later reconciliation surfaces only the current deferred revision with existing bounded surfaced/reminder semantics; resolved or superseded decisions do not resurface.

Ordinary Asana task + `go` remains autonomous: agents execute routine design/correction/dispatch and already-approved work without converting it into live approval ceremony. If local work is genuinely required, use the task-addressed durable local handoff/continuation rather than requiring Marco relay. Interrupt only for the actual current human-only decision or another standing non-agent boundary.
...
A manual Worker never mutates source while Review authority is active. Before dispatch, Code Review classifies each blocker against the accepted design (on a pilot lineage, using the finding-disposition/remedy-scope axes above). If it can be resolved within that design — or is `BLOCKER × S0` on a pilot lineage — after durably submitting and verifying the formal exact-head `VERDICT: BLOCK`, Review is complete and the **same Worker must then explicitly switch to current Implementation authority without another Marco prompt**, bind the live exact task/PR/branch/blocked head plus that formal BLOCK review ID, and fix only that accepted/S0 scope on the same PR lineage. If correctness requires a new or changed design requirement — or the pilot remedy is `S1`/`S2`/`MATERIAL UNKNOWN` — stop the PR and return the work to Design/the routing above; do not dispatch Implementation or convert the discovery into implementation scope. If any candidate identity moved, perform zero semantic mutation and reclassify current state.
...
Review routing is an explicit action of the acting Coordinator/outer worker. It re-reads the live PR, owning task, exact head, existing claims/reviews, and any durable review-class marker before dispatch. A local Claude Code/Codex worker with subagents available may immediately dispatch a fresh independent subagent as the Reviewer for a locally or remotely authored head. Independence belongs to the reviewing execution: that subagent must not have materially authored the candidate and receives Review authority only, while the outer worker retains routine lifecycle orchestration. If no such independent local execution is available, ordinary substantive/domain Review uses a fresh ChatGPT Review session. Missing or ambiguous candidate identity still fails closed; local authorship by the outer worker is not itself a blocker when the dispatched Reviewer is genuinely fresh and independent under `operator-provenance.md`. `domain:<name>` (e.g. `domain:postgresql`) deepens scrutiny inside the same Review role rather than selecting a second generic AI reviewer. Legacy durable `specialist:<name>` markers normalize to `domain:<name>`. Durable review-class markers may use `REVIEW CLASS: <class>` or `<!-- dish-review-route:v1 head=<sha> class=<class> -->`. The formal exact-head GitHub `COMMENT` review remains the completion artifact.
...
After an exact-head `BLOCK` that Review has classified as resolvable within the accepted design — or, on a pilot lineage, `BLOCKER × S0` — default the fix to `CHATGPT_IMPLEMENTATION`. A blocker requiring a new or changed design requirement, or a pilot `S1`/`S2`/`MATERIAL UNKNOWN` remedy, returns to Design/the routing above instead of entering Implementation host routing. Select `LOCAL_IMPLEMENTATION` only when the exact Review itself carries the canonical `IMPLEMENTATION / PUBLICATION — <exact unavailable remote capability>; fallbacks exhausted: <bounded list>` classification. A local Review does not implicitly keep the fix local, and an unavailable ChatGPT consumer does not fall back to local Implementation. #95 remains the sole post-PR mutation admission: the selected host maps to one broker route, the grant binds that accepted route, and any returned new head requires fresh independent Review. The execution that performed the fix cannot satisfy that next Review merely by declaring itself separate.

### Post-BLOCK correction transition

Review authority is read-only with respect to candidate semantics until the exact PASS/BLOCK verdict is durably submitted and verified. `VERDICT: MERGE` acquires no correction authority. After a durable exact-candidate `VERDICT: BLOCK`, an explicitly initiated manual Code Review/correction execution attempts a correction only when Review already classified it as an implementation defect resolvable within the accepted design — or, on a pilot lineage, `BLOCKER × S0`. A Design Review blocker, Code Review evidence that correctness requires a new or changed design requirement, or a pilot `S1`/`S2`/`MATERIAL UNKNOWN` remedy, returns to Design/the routing above as a new exact candidate; reviewer suggestions remain non-authoritative.

Before any implementation correction mutation, reread the exact current task/candidate/BLOCK and current authority, explicitly leave Review authority, and load Implementation as the replacement active authority. The accepted in-design (or pilot S0) code fix remains bound to the exact `(task, PR, branch, blocked head, formal BLOCK review id)` lineage. Scope classification is complete before dispatch: the fix agent does not challenge, reinterpret, or redefine the reviewer's blocker — except on a pilot lineage, where it re-grounds the governing baseline/accepted slice and may reclassify into S1/S2/material unknown on fresh evidence, stopping before widening rather than silently enlarging scope. `Agent owner`, Code Area, original author, `domain:*`, or specialist labels are context only and never create or require correction authority by themselves.
...
After material correction, persist/publish/freeze the successor with exact readback and stop Review work in that authoring execution: it cannot independently Review/PASS its successor. A fresh independent Review is required. The acting local Claude Code/Codex outer worker may dispatch that fresh Reviewer immediately and, after its durable verdict, continue the already-authorized phase progression through Integration/merge. This authorizes orchestration and fresh-agent dispatch, never self-review or semantic work under Review/Integration authority. A manually supplied batch is transport convenience only: each item retains separate task/candidate/BLOCK authority and one hard item does not stall unrelated safely correctable items.
...
After an isolated implementation blocker fix, a fresh independent reviewer normally rechecks that blocker and regressions caused by the fix on the new exact head. Freshness/independence is mandatory after material authorship; it is not permission for an unbounded fresh audit or wishlist. A genuinely new material defect may still block; on a pilot lineage the reviewer also judges the full accumulated candidate and classifies it and any remedy on the two independent axes before routing. If resolving it requires a new or changed design requirement, stop the PR and return to Design rather than enlarging the implementation.

### Scope-amplification check

Re-anchor the candidate to the original operator request or current accepted task specification before judging architectural completeness. Ask whether the solution materially expands beyond the smallest sufficient operator outcome. A materially large design, major scope expansion, or new subsystem/mechanism—including a scheduler, queue, database, service, ownership/identity system, control plane, or materially broader lifecycle—requires explicit durable Marco sign-off before it can receive `VERDICT: PASS` or become `IMPLEMENTATION READY`; without it, block the extra scope while preserving the narrow authorized slice. On a pilot lineage, apply the `S1`/`S2` necessity-review and Marco-decision routing above instead of rejecting on size/mechanism class alone.
