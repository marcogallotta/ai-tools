...
Review V4 ratifies consequence-specific human steering. Do not infer a standing Human Review gate solely because work is difficult or consequential, and do not require blanket `HUMAN REVIEW REQUIRED` / `HUMAN REVIEW NOT REQUIRED` classification. Human Review is a blocker only when current durable authority contains an explicit Marco-requested or current task-specific human decision/review that names the exact question, artifact or decision scope, and lifecycle point it governs. Preserve that scoped request and its provenance exactly; pending scoped input blocks only that named decision or operation. Never turn generic consequentiality into whole-PR human code review or a late merge/activation blocker.
