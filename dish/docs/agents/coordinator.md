...
Human Review is a live decision exchange, not a queueing ceremony. When research/design/Review reaches a genuine Marco-owned design, architecture, intent, authority, product, security, risk, or major-cost choice, surface it **before further design/freeze work on that choice**. Independently state the real decision, minimum evidence, meaningful alternatives/tradeoff, recommendation when defensible, and material consequence. Merely paraphrasing Marco's requested outcome and asking him to approve it is not Human Review.

If Marco answers in that interaction, persist the exact decision immediately on the owning task and continue under that durable authority. A later frozen generation binds the live decision and must not ask Marco to re-approve the same unchanged choice. A material consequential delta outside the recorded decision is a new Human Review and is surfaced immediately when discovered.

Only Marco's explicit deferral such as `not now` may park that exact unresolved decision in `Needs Human Review`. Persistence and presentation are one control flow: write the exact pending decision/revision, project `Needs Human Review`, then present that same review in the current interaction; never terminate at `queued for Human Review`. Startup/status reconciliation of a genuinely deferred item surfaces its **current exact unresolved decision revision** before lower-value work. Reuse the existing operator-decision surfaced-state/readback mechanics (`scripts/operator_decision.py`) to prevent repeated spam: one initial surface, the bounded existing reminder, and a changed question as a new revision. Resolved, superseded, stale, or non-current revisions do not resurface. Never infer resolution from account attribution.

Ordinary Asana task + `go`, routine correction, reversible engineering, and already-approved design execution remain autonomous and agent-owned. They do not manufacture Human Review. The only live interruption is the actual current Marco-owned choice (or another standing non-agent boundary); a required local continuation uses the task-addressed durable handoff rather than forcing Marco to relay agent work.
...
- Claude Code/Codex use local git/worktrees as appropriate; ChatGPT uses only the active Project's
  authorized, mutually exclusive GitHub route defined in
...
Consequence-specific Human Review is required when the live work reaches a genuine Marco-owned choice; difficulty, severity, uncertainty, metadata, or consequentiality by itself is not enough. Do not require blanket `HUMAN REVIEW REQUIRED` / `HUMAN REVIEW NOT REQUIRED` classification, whole-PR human code review, or a late merge/activation approval for routine agent-owned work. Pending Human Review blocks only the affected semantic scope while the exact choice is unresolved.

Treat Human Review as satisfied only by durable evidence of Marco's explicit answer to that exact decision scope. Persist a live answer immediately; authenticated-account attribution, workflow status, agent assertions, or a later paraphrase are not substitutes. Once the exact choice is durably resolved, a later design freeze or review may consume that decision without ceremonial re-approval. Only a material consequential delta outside the recorded scope creates a fresh Human Review.
