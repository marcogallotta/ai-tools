...
## Evidence-based execution-host selection

Host choice is separate from semantic role and elapsed runtime. Default substantive repository Implementation to the hosted/ChatGPT path. A local route may be recommended when current evidence names either an exact unavailable hosted capability with exhausted bounded fallbacks or a concrete material local advantage: required infrastructure/system capability, a proved large-file/payload/model-context boundary, required shared local state, or materially lower convergence/publication cost. Overlap, convenience, prior local-agent involvement, duration, command timeout, or preference alone is insufficient. Record the evidence and expected benefit.

`LOCAL BENEFIT` is a recommendation only: it creates no dependency or WAIT, does not change SEND NOW eligibility, and grants no local-dispatch, role, branch, Review, or Integration authority. Slow/native tests remain `TESTS ONLY`; sudo/systemd/installed-runtime operations remain `LOCAL SYSTEM ACCESS` unless semantic source mutation is also required. When no concrete material advantage exists, use the normal hosted path.
...
Routing is deliberately bounded and role+host aware. Ordinary semantic/domain Review uses the published ChatGPT Review Workspace Agent. Cheap mechanical/focused/light Review may use the configured local reviewer only when a positive exact-current-head witness proves the implementation came from `CHATGPT_IMPLEMENTATION`; local-authored or unknown provenance forces ChatGPT Review. A local Review host performs Review-authorized local evidence directly when capable; semantic/source changes route back through Implementation selection and Integration-only actions stay with Integration. Locality never grants another role's authority.

After exact-head `BLOCK`, the existing implementation/fix ownership path consumes the durable queue transition and updates the same PR branch; Marco does not forward the review transcript. The default fix host is `CHATGPT_IMPLEMENTATION`; `LOCAL_IMPLEMENTATION` may be recommended only from the current evidence-based host classification above, and ChatGPT consumer failure alone never falls back to local. The dispatcher re-reads the exact task/PR/branch/head/Review state before direct dispatch, then relies on existing advisory leases, branch/worktree ownership, and non-force expected-head publication to reject stale duplicate work. After exact-head `MERGE`, the dispatcher evaluates local work and Integration gates. When landing or mechanically determined reconciliation is ready, it writes/re-reads one exact-head local Integration handoff, acquires the repository-owned local per-PR/head fence, and invokes only the configured local Claude/Codex Integration launcher. The dispatcher never merges and never falls back to ChatGPT/connector/Actions Integration. If the local launcher is unavailable, the PR remains `INTEGRATION READY`. After the child returns, only authoritative GitHub `MERGED` readback permits scoped Asana reconciliation/cleanup; a head change returns to fresh exact-head Review. Likewise, Implementation publication/review-ready claims require authoritative remote branch + real PR + exact-head readback; intended/local artifacts never substitute. Tool capability alone never grants authority.
