...
Routing remains bounded and role+host aware. A local Claude Code/Codex outer worker with subagents available immediately dispatches a fresh independent Review subagent for the exact head, including a locally authored head; the reviewing execution must not have materially authored the candidate. If that independent local route is unavailable, ordinary semantic/domain Review uses a fresh ChatGPT Review session. Unknown candidate identity still fails closed. Semantic/source changes return through Implementation and Integration-only actions stay with Integration. Locality never grants another role's semantic authority; it only permits the outer worker to dispatch separate role-bound executions and continue routine phase progression after their durable results.
...
Human steering is limited to genuine Marco-owned choices whose mechanism materially determines operator experience, architecture/control-plane shape, intent, authority/trust, irreversible compatibility, security/risk, product behavior, or major cost. Routine and reversible design remains agent-owned. When such a choice becomes live during research/design/Review, conduct Human Review immediately **before further design or freeze work on that choice**: independently frame the real decision, minimum evidence, meaningful alternatives/tradeoff, recommendation when defensible, and material consequence. Echoing Marco's requested outcome back as an approve/reject prompt is not a substantive review.

If Marco answers in that interaction, persist the exact decision immediately on the owning task and let later frozen generations bind it. Do not manufacture a second post-freeze approval for the same unchanged choice. A material consequential delta outside the recorded decision creates a new immediate Human Review. Only Marco's explicit deferral such as `not now` may leave the exact unresolved revision parked in `Needs Human Review`; when deferring, persistence, section projection, and presentation are one control flow rather than `queued for Human Review` followed by silence. A later reconciliation surfaces only the current deferred revision, using existing surfaced/reminder mechanics where applicable, and never resurfaces a resolved or superseded decision.

Ordinary Asana task + `go` remains autonomous, including routine research/design/correction and already-approved execution. It stays autonomous until an actual human-only choice is reached; that affected scope pauses for the live decision while unrelated agent-owned work need not.
