        ...
        BEGIN
          ...
          IF TG_OP='UPDATE' THEN
            IF NEW.dish_version <> OLD.dish_version + 1
               ...
               OR (NOT receipt.completion_changed AND (NEW.completed IS DISTINCT FROM OLD.completed OR NEW.completion_reason IS DISTINCT FROM OLD.completion_reason OR NEW.archived_at IS DISTINCT FROM OLD.archived_at))
          ...
          IF NOT EXISTS (SELECT 1 FROM dish_mutation_receipts r
              WHERE r.generation_id=NEW.generation_id AND r.task_id=NEW.task_id
                ...
                AND ((r.source_route='import' AND NEW.completion_reason='imported')
                  OR (r.source_route='command_execution' AND (NEW.completion_reason IN ('cooked','archive','reopen_planning'){archive_source}))))
...
def _replace_sqlite_guard(*, independent: bool) -> None:
    archive_change = " OR NEW.archived_at IS NOT OLD.archived_at" if independent else ""
    ...
    op.execute(f"""
        CREATE TRIGGER dish_states_validate_update BEFORE UPDATE ON dish_states WHEN
          ...
          (NEW.completion_version=OLD.completion_version AND
            (NEW.completed<>OLD.completed OR NEW.completion_reason<>OLD.completion_reason{archive_change})) OR
            ...
            WHERE r.generation_id=NEW.generation_id AND r.task_id=NEW.task_id
              ...
              AND ((r.source_route='import' AND NEW.completion_reason='imported')
                OR (r.source_route='command_execution' AND
                  (NEW.completion_reason IN ('cooked','archive','reopen_planning'){archive_change}))))
