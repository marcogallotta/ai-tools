...
def _replace_postgresql_guard(*, include_archived_at: bool) -> None:
    archived_guard = " OR NEW.archived_at IS DISTINCT FROM OLD.archived_at" if include_archived_at else ""
    op.execute(f"""
        ...
        BEGIN
          ...
          IF TG_OP='UPDATE' THEN
            IF NEW.dish_version <> OLD.dish_version + 1
               ...
               OR (NOT receipt.completion_changed AND (NEW.completed IS DISTINCT FROM OLD.completed OR NEW.completion_reason IS DISTINCT FROM OLD.completion_reason {archived_guard}))
...
def upgrade() -> None:
    ...
    op.add_column("dish_states", sa.Column("archived_at", sa.DateTime(timezone=True), nullable=True))
    op.execute("UPDATE dish_states SET archived_at=updated_at WHERE completed AND completion_reason='archive'")
    ...
    op.create_index("ix_dish_states_archive", "dish_states", ["generation_id", "archived_at", "task_id"])
    if dialect == "postgresql":
...
def downgrade() -> None:
    ...
    if dialect == "postgresql":
        op.drop_constraint("ck_dish_states_archived_at_matches_completion", "dish_states", type_="check")
