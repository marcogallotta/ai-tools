...
RETIRED_HISTORY_IMPORT_MESSAGE = (
    "one-off Cooking History import is retired; use the native PostgreSQL Cooking History migration path"
)


def migrate_missing_history(
    session: Session,
    records: list[dict],
    *,
    source_sha256: str,
    cursor_secret: bytes,
    expected_count: int = 237,
    now: datetime | None = None,
) -> dict[str, int]:
    """Fail closed for the retired importer that created tasks without native placement."""
    raise RuntimeError(RETIRED_HISTORY_IMPORT_MESSAGE)
...
def main() -> None:
    ...
    if args.import_file:
        parser.error(RETIRED_HISTORY_IMPORT_MESSAGE)
    engine = create_database_engine(DatabaseSettings(url=os.environ["DISH_PG_URL"]))
    with Session(engine) as session, session.begin():
        report = reconcile_existing_history(
            session,
            args.gid,
            cursor_secret=os.environ["DISH_PG_CURSOR_SECRET"].encode(),
        )
