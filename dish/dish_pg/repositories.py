...
def compute_attestation_sha256(
    *,
    ...
    """Deterministic canonical hash for a runtime attestation row.

    Revision 1 hashes the baseline migration's provenance in place of a
    predecessor; successors hash their predecessor identity instead.  The
    serializer version is fixed so the hash is stable across code changes
    that do not alter the identity fields themselves.
    """
    ...
    payload = {
        ...
        "predecessor_attestation_id": (
            str(predecessor_attestation_id) if predecessor_attestation_id is not None else None
        ),
        "baseline_migration_event_id": (
            str(baseline_migration_event_id) if baseline_migration_event_id is not None else None
        ),
...
class RuntimeAuthorityRepository:
    """The one-way native-catalog runtime authority switch.

    Establishing revision 1 here is the sole moment legacy runtime authority
    is superseded.  This repository never creates or mutates
    ``AuthorityGeneration`` or ``HonestContractBinding``, and never depends on
    ``AuthorityActivation``.
    """
    ...
    def establish_root(
        self,
        *,
        migration_event: models.AppliedMigrationEvent,
        attestation_id: uuid.UUID,
        recorded_at: datetime,
    ) -> models.NativeCatalogRuntimeAttestation:
        """Insert revision-1 attestation + current pointer in the caller's transaction.

        The caller is responsible for committing the migration event, this
        attestation, and the current-pointer row as one atomic unit, and for
        supplying an ``AppliedMigrationEvent`` whose ``details`` already carry
        the required ``inventory_gate`` and provenance fields.
        """

        if migration_event.outcome != "applied":
            raise CoreAuthorityError("runtime authority root requires an applied migration event")

        generation = self.session.scalar(
            select(models.AuthorityGeneration)
            .where(models.AuthorityGeneration.generation_id == migration_event.generation_id)
            .with_for_update()
            .execution_options(populate_existing=True)
        )
        if generation is None or generation.status != "active":
            raise CoreAuthorityError("runtime authority root requires the active authority generation")

        existing_pointer = self.session.scalar(
            select(models.CurrentNativeCatalogRuntime)
            .where(models.CurrentNativeCatalogRuntime.generation_id == generation.generation_id)
            .with_for_update()
            .execution_options(populate_existing=True)
        )
        if existing_pointer is not None:
            raise CoreAuthorityError("runtime authority root already established for this generation")

        contract = CatalogRepository(self.session).active_catalog_contract(generation.generation_id)

        details = migration_event.details or {}
        gate = details.get("inventory_gate")
        if not isinstance(gate, dict) or gate.get("decision") not in (
            "no_carry_forward_required",
            "carry_forward_completed",
        ):
            raise CoreAuthorityError("runtime authority root requires a resolved inventory gate")
        if gate.get("decision") == "carry_forward_completed" and not gate.get("migration_event_id"):
            raise CoreAuthorityError(
                "runtime authority root requires the applied carry-forward migration event identity"
            )
        if (
            details.get("catalog_activation_id") != str(contract.catalog_activation.catalog_activation_id)
            or details.get("catalog_version_id") != str(contract.catalog_version.catalog_version_id)
            or details.get("honest_contract_binding_id") != str(contract.honest_binding.binding_id)
        ):
            raise CoreAuthorityError(
                "runtime authority root migration event provenance does not match the active catalog"
            )

        attestation_sha256 = compute_attestation_sha256(
            ...
            baseline_migration_event_id=migration_event.migration_event_id,
            baseline_revision=migration_event.revision,
            baseline_migration_code_sha256=migration_event.migration_code_sha256,
            baseline_dish_release=migration_event.dish_release,
            baseline_source_commit_sha=details.get("source_commit_sha"),
        )
        ...
        readback_hash = compute_attestation_sha256(
        ...
        if readback_hash != attestation.attestation_sha256:
            raise CoreAuthorityError("runtime authority root hash readback mismatch")
        return attestation

    def current_runtime(
        self, generation_id: uuid.UUID
    ) -> tuple[models.CurrentNativeCatalogRuntime, models.NativeCatalogRuntimeAttestation]:
        """Resolve the durable native runtime root; never falls back to legacy authority."""

        pointer = self.session.get(models.CurrentNativeCatalogRuntime, generation_id)
        if pointer is None:
            raise CoreAuthorityError("no native runtime authority root exists for this generation")
        attestation = self.session.get(
            models.NativeCatalogRuntimeAttestation, pointer.attestation_id
        )
        if (
            attestation is None
            ...
            raise CoreAuthorityError("native runtime authority pointer is inconsistent")
