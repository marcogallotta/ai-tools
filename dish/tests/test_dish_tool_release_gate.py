...
def test_large_cycle_freezes_current_release_and_preserves_all_run_lineage(tmp_path):
    ...
    first = app.execute("start", agent="codex", task_gid="t", kind="verification", run_id="editor-run")
    assert first["ok"]
    ...
    barred = app.execute("start", agent="gpt", task_gid="t", kind="verification", run_id="constructor-run")
...
def test_governed_lock_change_requires_human_authorization(tmp_path):
    ...
    review = app.execute("start", agent="codex", task_gid="t", kind="verification", run_id="review-run")
    assert review["ok"]
