...
def test_task_level_constructor_run_remains_barred(tmp_path):
    ...
    # A later operation on the same task must still remember the constructor.
    ...
    from dish_tool.database import create_operation
    from dish_tool.models import OperationActors
    op2 = create_operation(app.conn, task_gid="t", operation_kind="change", expected_identity=app.conn.execute("SELECT last_confirmed_identity FROM task_content_state WHERE task_gid='t'").fetchone()[0], schema_version="2", actors=OperationActors(editor_agent="codex", run_id="later-editor"))
    from dish_tool.database import assert_fresh_verifier
    import pytest
    with pytest.raises(Exception) as exc:
        assert_fresh_verifier(app.conn, operation_id=op2["operation_id"], agent="gpt", run_id="constructor-run", independence_attestation=None)
    assert getattr(exc.value, "rule", None) == "verifier_not_independent"
