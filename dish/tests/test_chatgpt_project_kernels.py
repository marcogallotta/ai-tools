...
def _obs(sid,role):
 ...
 if sid in {'chatty-continue-resumes-active-work','chatty-premature-stop-correction-resumes'}:
  target='continue-fixture' if sid=='chatty-continue-resumes-active-work' else 'resume-fixture'
  return [
   {'seq':1,'kind':'durable_write','operation':'chatty_disposable_action','target':target,'write_id':f'{sid}-write'},
   {'seq':2,'kind':'readback','operation':'chatty_disposable_action','target':target,'write_id':f'{sid}-write','verified':True}]
 return []
def _passing():
 m,_=kernels.load_canonical(); results=[]
 for s in kernels._evals():
  for role in s['roles']:
   result={'case_id':f"{s['id']}::{role}",'fresh_chat_id':f"chat-{s['id']}-{role}",'assistant_response':{'outcome':s['expected_outcome'],'actions':list(s['required_actions'])},'runner_observations':_obs(s['id'],role)}
   if 'transcript_quality' in s:
    result['transcript_quality']={'grade':'pass','failure_modes':[],'metrics':{'operator_turns':1,'tool_chatter_events':0,'assistant_tokens':80}}
...
def test_project_settings_budget_change_requires_bound_evidence():
 ...
 with pytest.raises(kernels.KernelError,match='bind max_project_settings_chars'):
  kernels.project_settings_policy(unbound)
 unsupported=copy.deepcopy(m); unsupported['max_project_settings_chars']=8500; unsupported['project_settings_compatibility']['qualified_chars']=8500
 with pytest.raises(kernels.KernelError,match='requires empirical Project save/load/readback or official Project-limit evidence'):
  kernels.project_settings_policy(unsupported)
 supported=copy.deepcopy(unsupported); supported['project_settings_compatibility'].update({'basis':'official-project-limit','evidence_ref':'official:test'})
 assert kernels.project_settings_policy(supported)==8500
...
def test_scope_proportionality_keeps_one_intent_chain_and_all_g2_regressions():
 m,s=kernels.load_canonical()
 procedure=(DISH_ROOT/'docs'/'agents'/'scope-proportionality.md').read_text()
 implementation=(DISH_ROOT/'docs'/'agents'/'implementation.md').read_text()
 ...
 for token in (
  'sole authority', 'disposable working projections', 'calibrates', 'SHRINK AND CONTINUE',
  'JUSTIFIED EXPANSION', 'SEPARATE DEPENDENCY / OWNER', 'tripwires only',
  'no persisted four-section plan', 'second attempt', 'high-consequence',
 ):
  assert token in procedure
 assert 'scope-proportionality.md' in implementation
 assert 'scope-proportionality.md' in review
 assert 'One exact governing Marco Intent Baseline/specification' in lifecycle
 for role in ('coordinator','development-workflow','implementation','review'):
  rule=next(x for x in s['roles'][role]['rules'] if x['id']=='scope-amplification-checkpoint')
  assert 'one governing Intent Baseline' in rule['text']
  assert 'Metrics are tripwires only' in rule['text']
 expected={
  'scope-baseline-rejects-historical-reconstruction',
  'scope-missing-baseline-routes-authority',
  'scope-one-baseline-before-during-after',
  'scope-historical-future-remains-reserved',
  'scope-trivial-change-no-mandatory-plan',
  'scope-small-high-consequence-keeps-controls',
  'scope-second-attempt-must-change',
  'scope-necessary-complexity-routes-design',
  'scope-generated-volume-tripwire-only',
  'scope-new-control-plane-braked',
 }
 for scenario_id in expected:
  assert _scenario(scenario_id)['required_rules']==['scope-amplification-checkpoint']
...
def test_chatty_contract_is_canonical_in_root_and_projects_reference_it_after_startup():
 ...
 assert any('Match the operator job before depth' in x for x in rules)
 assert any('do not force empty owner/risk/action categories' in x for x in rules)
 assert any('continue` or `resume' in x and 'actionable work remains' in x for x in rules)
 assert any('handoff as normal task input' in x and 'Silently ground' in x for x in rules)
 assert any('Ask one bounded question only' in x for x in rules)
 for role in s['roles']:
  text=kernels.render_role(m,s,role)
  assert text.index('Work chat:') < text.index(f"Role: **{s['roles'][role]['default_role']}**.")
  assert 'apply root `CLAUDE.md` `## Work chat`' in text
  assert 'lead with result/action/blocker/decision' in text
  for rule in rules: assert f'- {rule}' not in text
...
def test_chatty_transcript_quality_gates_are_hidden_and_enforced():
 bundle=kernels.prepare_eval_bundle()
 assert all('transcript_quality' not in case for case in bundle['cases'])
 assert next(x for x in bundle['cases'] if x['case_id']=='chatty-continue-resumes-active-work::implementation')['transcript_quality_required'] is True
 assert 'transcript_quality_required' not in next(x for x in bundle['cases'] if x['case_id']=='stale-project-version::implementation')
 assert set(bundle['response_contract']['transcript_quality_shape']['metrics'])==kernels.TRANSCRIPT_METRICS
 payload=_passing(); kernels.evaluate_behavior_results(payload)
 target=next(x for x in payload['results'] if x['case_id']=='chatty-continue-resumes-active-work::implementation')
 target['transcript_quality']['failure_modes']=['premature_stop']
 with pytest.raises(kernels.KernelError,match='forbidden transcript failure mode'):
  kernels.evaluate_behavior_results(payload)

def test_chatty_transcript_metrics_reject_avoidable_operator_turns():
 payload=_passing(); target=next(x for x in payload['results'] if x['case_id']=='chatty-routine-choice-no-escalation::implementation')
 target['transcript_quality']['metrics']['operator_turns']=2
 with pytest.raises(kernels.KernelError,match='operator_turns exceeds 1'):
  kernels.evaluate_behavior_results(payload)
...
def test_coordinator_concurrency_requires_evidence_and_preserves_reasoning_frontier():
 ...
 for text in (coordinator,architecture):
  ...
  assert 'exact downstream work' in text
  assert 'new material evidence' in text
  assert 'P-CRITICAL/P0' in text
 assert 'ambiguous `SEND NEXT`' in coordinator
 expected={
  'coordinator-concurrency-nonoverlap-start-wait',
  'coordinator-concurrency-exact-sounding-noncause',
  'coordinator-concurrency-true-causal-wait',
