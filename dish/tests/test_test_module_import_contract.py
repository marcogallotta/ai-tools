...
def _collected_test_modules() -> set[str]:
    root = Path(__file__).parent
    return {
        "tests." + ".".join(path.relative_to(root).with_suffix("").parts)
...
def test_tests_do_not_import_other_test_modules():
    ...
    root = Path(__file__).parent
    test_modules = _collected_test_modules()
    violations: list[str] = []
    ...
    for path in sorted(root.rglob("*.py")):
        ...
        for node in ast.walk(tree):
            ...
            for module in imported:
                if any(module == candidate or module.startswith(candidate + ".") for candidate in test_modules):
                    violations.append(
