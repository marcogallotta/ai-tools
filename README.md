...
Reads the API token from `~/.config/asana-cli/.env`. The hook allows mechanically proven routine
writes on an agent's exact active task; unscoped or discretionary writes still require approval.
