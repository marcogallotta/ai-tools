...
def _active_watchers() -> list[int]:
    try:
        result = subprocess.run(["ps", "-eo", "pid=", "-o", "args="], check=False, capture_output=True, text=True, timeout=3)
    except (OSError, subprocess.SubprocessError):
        return []
    found = []
...
def _render(value: dict[str, Any]) -> str:
    ...
    for key in ("supervisor_pid", "watcher_pid"):
        if value.get(key): fields.append(f"{key}={value[key]}")
    fields.append(f"log={value['log_path']}")
    if value.get("projection_path"):
        fields.append(f"projection={value['projection_path']}")
    if value.get("unmanaged_watchers"): fields.append("unmanaged_watchers=" + ",".join(map(str, value["unmanaged_watchers"])))
    if value.get("restart_count"): fields.append(f"restarts={value['restart_count']}")
    if value.get("last_error"): fields.append(f"last_error={value['last_error']}")
    return " | ".join(fields)


def _terminate(pid: int, verify, state: dict[str, Any], timeout: float = 5.0) -> None:
    if not verify(pid, state): raise ControllerError(f"refusing to signal unverified PID {pid}")
    os.kill(pid, signal.SIGTERM)
    deadline = time.monotonic() + timeout
    while _alive(pid) and time.monotonic() < deadline: time.sleep(0.1)
    if _alive(pid):
        if not verify(pid, state): raise ControllerError(f"PID {pid} changed identity while stopping")
        os.kill(pid, signal.SIGKILL)
        deadline = time.monotonic() + 2
        while _alive(pid) and time.monotonic() < deadline: time.sleep(0.05)
        if _alive(pid): raise ControllerError(f"verified PID {pid} did not stop after SIGKILL")


def start(paths: dict[str, Path]) -> int:
    with _lock(paths):
        state = _state(paths); old_pid = _pid(paths)
        if old_pid and _owned_supervisor(old_pid, state):
            print(_render(_snapshot(paths))); return 1
        if old_pid and _alive(old_pid): raise ControllerError(f"PID file points to live unowned PID {old_pid}")
        old_watcher = state.get("watcher_pid")
        if isinstance(old_watcher, int) and _alive(old_watcher):
            if _owned_watcher(old_watcher, state): _terminate(old_watcher, _owned_watcher, state, 2)
            else: raise ControllerError(f"stale state points to live unowned watcher PID {old_watcher}")
        unmanaged = _active_watchers()
        if unmanaged: raise ControllerError(f"active unmanaged lifecycle watcher(s) {unmanaged}; refusing duplicate start")
        run_id = uuid.uuid4().hex; _ensure(paths)
        paths["log"].touch(exist_ok=True, mode=0o600); paths["log"].chmod(0o600)
        _save(paths, schema="dish-pr-lifecycle-controller-state-v1", run_id=run_id, status="starting", supervisor_pid=None, watcher_pid=None, restart_count=0, last_error=None)
        command = [sys.executable, str(Path(__file__).resolve()), "--state-dir", str(paths["root"]), "_supervise", "--run-id", run_id]
        with paths["log"].open("ab", buffering=0) as log:
            process = subprocess.Popen(command, stdin=subprocess.DEVNULL, stdout=log, stderr=log, cwd=str(ROOT), start_new_session=True, close_fds=True)
        birth, _ = _ps(process.pid); _atomic(paths["pid"], f"{process.pid}\n"); _save(paths, supervisor_pid=process.pid, supervisor_birth=birth)
    deadline = time.monotonic() + 5
    while time.monotonic() < deadline:
        value = _snapshot(paths)
        if value["supervisor_live"] and value["status"] in {"running", "reconnecting"}: print(_render(value)); return 0
        if value["status"] == "failed" or not _alive(process.pid): raise ControllerError(_render(value))
        time.sleep(0.05)
...
def stop(paths: dict[str, Path]) -> int:
    with _lock(paths):
        state = _state(paths); pid = _pid(paths)
        if not pid or not _alive(pid):
            paths["pid"].unlink(missing_ok=True); _save(paths, status="stopped", supervisor_pid=None, watcher_pid=None); print(_render(_snapshot(paths))); return 0
        _save(paths, status="stopping"); _terminate(pid, _owned_supervisor, state, 10)
        watcher = state.get("watcher_pid")
        if isinstance(watcher, int) and _alive(watcher): _terminate(watcher, _owned_watcher, state, 2)
        paths["pid"].unlink(missing_ok=True); _save(paths, status="stopped", supervisor_pid=None, watcher_pid=None)
    print(_render(_snapshot(paths))); return 0
...
def _await_supervisor_binding(paths: dict[str, Path], run_id: str, timeout: float = 5.0) -> dict[str, Any]:
    ...
    while True:
        state = _state(paths)
        if _pid(paths) == os.getpid() and state.get("run_id") == run_id and state.get("supervisor_pid") == os.getpid(): return state
        if time.monotonic() >= deadline: raise ControllerError("supervisor identity does not match owned state")
        time.sleep(0.02)


def supervise(paths: dict[str, Path], run_id: str) -> int:
    _await_supervisor_binding(paths, run_id)
    stopping = False; child: subprocess.Popen[Any] | None = None
    ...
    while not stopping:
        ...
        try: child = subprocess.Popen(_watcher_command(paths), cwd=str(ROOT))
        except OSError as exc:
            crash_attempt += 1; transient_attempt = 0
            if crash_attempt > CRASH_RESTART_LIMIT:
                _save(paths, status="failed", watcher_pid=None, last_error=f"watcher launch failed: {exc}"); return 2
            restarts += 1; delay = _backoff(crash_attempt)
            _save(paths, status="reconnecting", watcher_pid=None, restart_count=restarts, next_retry_seconds=delay, last_error=f"watcher launch failed: {exc}")
            time.sleep(delay); continue
        birth, _ = _ps(child.pid)
        _save(paths, status="running", watcher_pid=child.pid, watcher_birth=birth, restart_count=restarts, next_retry_seconds=None, last_error=None)
        code = child.wait(); child = None
        if stopping: break
        if time.monotonic() - started >= INTERVAL: transient_attempt = crash_attempt = 0
        error = _tail_error(paths["log"])
        if code == 2 and _transient(error): transient_attempt += 1; crash_attempt = 0
        elif code == 2:
            _save(paths, status="failed", watcher_pid=None, last_error=error or "watcher exited with code 2"); return 2
        else:
            crash_attempt += 1; transient_attempt = 0
            if crash_attempt > CRASH_RESTART_LIMIT:
                _save(paths, status="failed", watcher_pid=None, last_error=error or f"watcher repeatedly exited with code {code}"); return code or 2
        restarts += 1; delay = _backoff(transient_attempt or crash_attempt)
        _save(paths, status="reconnecting", watcher_pid=None, restart_count=restarts, next_retry_seconds=delay, last_error=error or f"watcher exited with code {code}")
        time.sleep(delay)
    _save(paths, status="stopped", watcher_pid=None); return 0


def logs(paths: dict[str, Path], lines: int, follow: bool) -> int:
    if lines < 0 or paths["log"].is_symlink(): raise ControllerError("invalid log request")
    if not paths["log"].exists(): raise ControllerError(f"log does not exist yet: {paths['log']}")
    content = paths["log"].read_text(encoding="utf-8", errors="replace").splitlines(True); sys.stdout.writelines(content[-lines:] if lines else [])
...
def main(argv: list[str] | None = None) -> int:
    parser = argparse.ArgumentParser(prog="dish-pr-lifecycle-controller"); parser.add_argument("--state-dir", type=Path, help=argparse.SUPPRESS)
    sub = parser.add_subparsers(dest="command", required=True)
    for name in ("start", "stop", "restart", "status"): sub.add_parser(name)
    log = sub.add_parser("logs"); log.add_argument("--lines", type=int, default=100); log.add_argument("--follow", action="store_true")
    internal = sub.add_parser("_supervise", help=argparse.SUPPRESS); internal.add_argument("--run-id", required=True, help=argparse.SUPPRESS)
    args = parser.parse_args(argv); paths = _paths(args.state_dir)
    try:
        if args.command == "start": return start(paths)
        if args.command == "stop": return stop(paths)
        if args.command == "restart": stop(paths); return start(paths)
        if args.command == "status":
            value = _snapshot(paths); print(_render(value)); return 0 if value["status"] in {"running", "reconnecting"} else 1
        if args.command == "logs": return logs(paths, args.lines, args.follow)
        return supervise(paths, args.run_id)
    except ControllerError as exc:
        print(f"dish-pr-lifecycle-controller: {exc}", file=sys.stderr); return 2
