...
Standing exceptions use current reviewed Git only; there is no ChatGPT Project-settings overlay or fresh-chat activation dependency. Apply [`dish/docs/agents/fast-track-process.md`](dish/docs/agents/fast-track-process.md) and current [`dish/docs/chatgpt-projects/fast-track-gates.json`](dish/docs/chatgpt-projects/fast-track-gates.json). A new exception or reactivation is unusable until its canonical Development Workflow P-CRITICAL same-day follow-up is created and authoritatively read back; repeated use creates no duplicate, while reactivation requires a new activation/follow-up. Bind exact gate/version/semantic digest and Marco provenance, preserve raw failure, record `GATE WAIVED BY MARCO OVERRIDE`, and never inherit future/materially changed gates or waive exact identity, independent Review, Integration separation, destructive/production safeguards, platform impossibilities, or other gates. Current-chat revocation is immediate.
...
- For a necessary manual relay, send one fenced locator-first copy block and only non-reconstructable payload. Inline payload must satisfy both limits: at most 8 non-empty lines and 700 characters. Above either limit, local Claude/Codex gets only an absolute path to the complete temp file; ChatGPT uses a supported transferable artifact or reports the exact transport blocker. A revision replaces the old handoff; no relay means no ceremony.
...
Claude Code and Codex use their live checkout plus their host-native Git/tooling and environment. For implementation/fix work, use the repository-owned `tools/agent-worktree` lifecycle rather than creating a competing branch/worktree or synchronizing the operator `main` checkout. For fresh authoring with an authoritative task, repository, and target but no existing lineage, orchestration/Implementation resolves and records the canonical branch plus exact base ref + SHA from live authority; Marco does not supply those routine fields. First creation still requires that recorded base to match `origin`. Resume/fix/publication work remains bound to its durable branch/PR/head/base, and resume observes current origin state without automatically resetting, merging, rebasing, or chasing a moved `main`. Enter the returned owned path directly or use `tools/agent-worktree exec --task <gid> -- <agent-command>`.
